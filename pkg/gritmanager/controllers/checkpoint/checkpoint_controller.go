...
func (c *Controller) checkpointingHandler(ctx context.Context, ckpt *v1alpha1.Checkpoint) error {
	...
	if err != nil || isFailed {
		ckpt.Status.Phase = v1alpha1.CheckpointFailed
		util.UpdateCondition(c.clock, &ckpt.Status.Conditions, metav1.ConditionTrue, string(v1alpha1.CheckpointFailed), "GritAgentJobFailed", fmt.Sprintf("grit agent job(%s/%s) is not found or failed in checkpointing state", gritAgentJob.Namespace, gritAgentJob.Name))
	}
	return nil
}
